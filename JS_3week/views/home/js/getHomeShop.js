...
class ShopList extends InitFactory{
  beforeMount() {
    emitter.on(ROLL_TO_BOTTOM,() =>this.mounted());
  }

  getData() {
    return getHomeShopData();
  }
  ...
  handleData() {
    ...
    data.forEach(e => {
      ...
      const item = tempItem
        ...
        .replace('__star__',this.getStar(wm_poi_score))
        .replace('__information__',this.getDiscount(discounts2))
        .replace('__brand__',brand_type ? '品牌' : '新店' )
        .replace('__brand-type__',brand_type ? 'old-brand' : 'new-brand');
      html += item;
      this.getStar(wm_poi_score);
      let shopTemp = temp
  ...
  mounted() {
    this.addList();
    this.getData().then(data=> this.handleData());
  }
